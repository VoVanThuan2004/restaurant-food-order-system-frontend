  ...
  return (
    <div className="bg-white rounded-xl border border-gray-300 shadow-md w-full px-5 py-4 mt-5 mx-auto">
      ...
      <div className="flex items-center justify-between mt-2">
        ...
        <p className="text-[17px]">{formatCurrency(order.amountReceived || 0)}</p>
      </div>

      {/* Tiền thối */}
      <div className="flex items-center justify-between mt-2">
        <p className="text-gray-500 text-[16px]">Tiền thối</p>
        <p className="text-[17px]">{formatCurrency(order.changeAmount || 0)}</p>
      </div>
      ...
      <div className="flex items-center mt-2 gap-2">
        ...
        <p className="text-gray-500">Time: {formatDateTime(order.paymentTime)}</p>
      </div>
